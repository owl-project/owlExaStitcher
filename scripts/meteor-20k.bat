...
set BRICKS=-bricks E:\stitcher\meteor-20060\meteor-20060.bricks
set SCALARS=-scalars E:\stitcher\meteor-20060\meteor-20060.exa.tev.scalars
set KDTREE=-kdtree E:\stitcher\meteor-20060\meteor-20060.kd

set CAMERA=--camera 2607.41 3218.14 3474.3 494.535 1300.13 -210.744 -0.242807 0.91051 -0.334689 -fovy 70
set XF=-xf meteor-20k.xf
...
FOR /L %%s IN (0,1,1) DO (
  FOR /L %%t IN (0,1,4) DO (
    ...
    %BUILD_DIR%\Release\exaStitchHeadlessViewer %BRICKS% %KDTREE% %SCALARS% %MESH% %CAMERA% %XF% %NUM_MCS% %LIGHT% %CLIP_PLANE% %IMG_SIZE% -o meteor-20k_sm%%s_tm%%t -fps meteor-20k.fps -rt 0 > meteor-20k_sm%%s_tm%%t.txt
  )
)
