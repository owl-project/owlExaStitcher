...
FOR /L %%s IN (0,1,1) DO (
  FOR /L %%t IN (0,1,4) DO (
    ...
    %BUILD_DIR%\Release\exaStitchHeadlessViewer %BRICKS% %KDTREE% %SCALARS% %MESH% %CAMERA% %XF% %NUM_MCS% %LIGHT% %CLIP_PLANE% %IMG_SIZE% -o cloud_sm%%s_tm%%t -rt 0 > cloud_sm%%s_tm%%t.txt
  )
)
