...
Array3D::Array3D(ExaStitchGlobalState *state, const Array3DMemoryDescriptor &d)
...
anari::uint3 Array3D::size() const
{
  return anari::uint3(uint32_t(size(0)), uint32_t(size(1)), uint32_t(size(2)));
}
...
EXA_ANARI_TYPEFOR_DEFINITION(exa::Array3D *);
