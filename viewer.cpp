...
namespace exa {
  ...
  public:
    ...
    void key(char key, const vec2i &where) override
    {
      ...
      //     SAHVolumeWrapper vol(model,&rgbaCM,renderer->xf.absDomain,renderer->xf.relDomain);
  ...
  extern "C" int main(int argc, char** argv)
  {
...
#if 0
    // TODO: test code for block-array c'tor
    ...
    std::vector<box3i> blockBounds(2);
    blockBounds[0] = {
    ...
    // The levels, per block
    std::vector<int> blockLevel(2);
    blockLevel[0] = 0;
    blockLevel[1] = 1;

    // Finally the data, for convenience we create a 1D array here
    // (that we need to keep alive!), and define the "blockData's"
    // to point into that
    std::vector<float> blockDataData(9);
    for (int i=0; i<8; ++i) blockDataData[i] = i/float(8);
    blockDataData[8] = 1.f;
    // (the "blockData's" themselves)
    std::vector<OWLRenderer::BlockData> blockData(2);
    blockData[0].size = vec3i(2,2,2);
    blockData[0].data = &blockDataData[0];
    blockData[1].size = vec3i(1,1,1);
    blockData[1].data = &blockDataData[8];

    OWLRenderer renderer(blockBounds.data(),
                         blockLevel.data(),
                         blockData.data(),
                         blockBounds.size(),
