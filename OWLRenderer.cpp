...
namespace exa {
  ...
  OWLRenderer::OWLRenderer(const ExaBrick *bricks, const float *scalars, size_t numBricks,
                           const box3f remap_from,
                           const box3f remap_to,
                           const vec3i numMCs)
  {
    model = ExaBrickModel::load(bricks,scalars,numBricks);
  ...
  OWLRenderer::OWLRenderer(const box3i *blockBounds, const int *blockLevel,
                           const BlockData *blockData, size_t numBlocks,
                           const box3f remap_from,
                           const box3f remap_to,
                           const vec3i numMCs)
  {
    throw std::runtime_error("block array constructor not implemented yet");
  }

  void OWLRenderer::initGPU()
  {
    // ==================================================================
    // Upload to GPU
    // ==================================================================
...
#ifdef EXA_STITCH_MIRROR_EXAJET
        owl4x3f tfm = model->mirrorTransform;
        ownMajorants.tlas = owlInstanceGroupCreate(owl, 2);
        owlInstanceGroupSetChild(ownMajorants.tlas, 0, ownMajorants.blas);
        owlInstanceGroupSetChild(ownMajorants.tlas, 1, ownMajorants.blas);
        owlInstanceGroupSetTransform(ownMajorants.tlas, 1, &tfm);
#else
  ...
  void OWLRenderer::setColorMap(const std::vector<vec4f> &newCM)
  {
    ...
    if (!xf.colorMapBuffer)
      ...
      prevSize = owlBufferSizeInBytes(xf.colorMapBuffer)/sizeof(vec4f);
      if (prevSize != newCM.size())
        owlBufferResize(xf.colorMapBuffer,newCM.size());
    }
    ...
    if (prevSize > 0 && prevSize != newCM.size()) {
      cudaFreeArray(xf.colorMapArray);
      xf.colorMapArray = 0;
    }
